...
class ajaxExport implements exportInterface
{
    ...
    public function start ()
    {
        ...
        $posts = $wpdb->get_col( $wpdb->prepare( "select ID from $wpdb->posts where post_type=%s and ID > %d order by ID limit %d",
            'product', $current_id, 10 ) );
    ...
    public function exportProduct ($post)
    {
        ...
        $className = "AEM\\exporter\\".$product->product_type;
