...
class woocsv_import_product
{
    ...
    */
    ...
    public function update_exist_post_by_title($newPost)
    {
        // just update wp_post's product row if it's title exist.
        $postTitle = $newPost['post_title'];
        $customPost = get_page_by_title($postTitle, OBJECT, 'product');

        if (!is_null($customPost)) {
            $exist_id = $customPost->ID;
            $newPost["ID"] = $exist_id;
            $new_post_id = wp_update_post($newPost, TRUE);
    ...
    public function save($parent_post_id)
    {
        ...
        if ($this->product["is_parent"]) {// Only for parent product.
            $this->save_parent_attributes($post_id);
        }
...
//            $this->save_product_gallery();
    ...
    public function save_parent_attributes($post_id)
    {
...
//        sleep(60);
        ...
        // FIXME: DJZHANG: add jpg extension.
        @preg_match('/[^\?]+\.(jpg|jpe|jpeg|gif|png|pdf)/i', $url . '.jpg', $matches);
        if (!$matches) {
    ...
    {
        // FIXME: DJZHANG: all urls are valid url.
